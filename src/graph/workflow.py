...
class WorkflowState(TypedDict, total=False):
    ...
    tests: pb.Tests
    patch: pb.Patch
    critique: pb.Critique
    repair_plan: pb.RepairPlan
    attempts: int


# --- Node implementations -------------------------------------------------

def spec_node(state: WorkflowState) -> dict:
...
def repair_node(state: WorkflowState) -> dict:
    with tracer.start_as_current_span("repair_agent"):
        ...
        return {
            "repair_plan": pb.RepairPlan(steps=[f"fix {attempt}"]),
            "attempts": attempt,
        }
